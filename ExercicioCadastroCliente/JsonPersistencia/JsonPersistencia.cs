...
namespace ExercicioCadastroCliente.JsonPersistencia
{
    public class PersistenciaJson<T> :IPersistencia<T>
    {
        ...
        public void Incluir(T entidade)
        {
            ...
            string stringJson = JsonSerializer.Serialize(entidades, new JsonSerializerOptions{ WriteIndented = true});
        ...
        private T descobrirObjeto(T entidade, List<T> entidades)
        {
            ...
            var propriedadeId = entidade.GetType()
                .GetProperties()
                .FirstOrDefault(p => Attribute.IsDefined(p, typeof(IdentidadeAttribute)));
            ...
            var objLocalizado = entidades.FirstOrDefault(e =>
            {
                if (e != null)
                {
                    var propriedadeIdLocalizado = e.GetType()
                    .GetProperties()
                    .FirstOrDefault(p => Attribute.IsDefined(p, typeof(IdentidadeAttribute)));
